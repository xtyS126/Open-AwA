...
.EXAMPLE
    .\bin\deploy.ps1
    .\bin\deploy.ps1 -Mode full
    .\bin\deploy.ps1 -Mode postgres -AdminUser admin -AdminPassword "MyStrong1Pass"
#>
...
switch ($Mode) {
    ...
    "prod" {
        $composeFileArgs = @("-f", "deploy/docker-compose.yml", "-f", "deploy/docker-compose.prod.yml")
        $profileArgs = @()
        Write-Info "生产模式需要 DOMAIN 环境变量，请在 .env 中配置后重新运行"
        Write-Info "首次申请 SSL 证书: bash deploy/init-ssl.sh"
    }
    "postgres" {
        $composeFileArgs = @("-f", "deploy/docker-compose.yml", "-f", "deploy/docker-compose.postgres.yml")
        $profileArgs = @()
    }
}
