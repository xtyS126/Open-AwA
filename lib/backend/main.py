...
async def _shutdown_plugin_system() -> None:
    ...
    manager = plugin_instance.reset()
    if manager is None:
        return

    for plugin_name in reversed(list(getattr(manager, "loaded_plugins", {}).keys())):
        try:
            await asyncio.to_thread(manager.unload_plugin, plugin_name)
        except Exception as exc:
            logger.bind(event="plugin_unload_error", module="shutdown", plugin=plugin_name).warning(
                f"插件卸载失败: {exc}"
            )
...
async def get_csrf_token(
    ...
    raw_token, _ = generate_csrf_token_pair(response)
...
async def csrf_protection_middleware(request: Request, call_next):
    ...
    # CSRF Cookie 由 /auth/login 与 /auth/csrf-token 端点成对签发，
    # 中间件不再单独轮换 Cookie（避免与前端内存中的 raw_token 失配，违反双提交模式硬约束）。
    return response
...
# 头像目录锚定到项目根 var/data/uploads/avatars，避免受进程 CWD 影响
# __file__ = lib/backend/main.py，parents[2] = 项目根
_AVATARS_DIR = FsPath(__file__).resolve().parents[2] / "var" / "data" / "uploads" / "avatars"
_AVATARS_DIR.mkdir(parents=True, exist_ok=True)
_avatars_dir = _AVATARS_DIR  # 兼容历史变量名引用
...
# ---- 前端静态文件服务（生产模式）----
# __file__ = lib/backend/main.py；parents[2] = 项目根；前端构建产物在 lib/frontend/dist
_project_root = FsPath(__file__).resolve().parents[2]
_FRONTEND_DIST = _project_root / "lib" / "frontend" / "dist"
