...
视频下载能力新增 4 张数据库表，ORM 模型定义在 `lib/backend/db/models/bilibili_toolkit.py`，
Alembic 迁移脚本位于 `lib/backend/migrations/versions/`。
...
路由定义在 `lib/backend/plugins/bilibili_toolkit_builtin/api/routes.py`。所有路由需登录鉴权。
...
视频下载能力向 Agent 暴露 5 个工具，定义在 `lib/backend/plugins/bilibili_toolkit_builtin/tools.py`，
...
pip install -r lib/backend/plugins/bilibili_toolkit_builtin/requirements.txt
