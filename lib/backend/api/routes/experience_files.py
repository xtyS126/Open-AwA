...
def _get_memory_skill_dir(user_id: int) -> Path:
    ...
    路径：项目根 / var / memory_skill / <user_id>
    说明：lib/backend/api/routes/experience_files.py 的 parents[4] 才是项目根目录。
    """
    memory_skill_dir = Path(__file__).resolve().parents[4] / "var" / "memory_skill" / str(user_id)
