...
* [x] SubTask 2.2: 更新第 1532-1555 行的子代理事件发射逻辑
  ...
  * 移除现有"未返回可追踪 agent\_id"的 warning，因为前台模式修复后将始终有 agent\_id

**验证**: 前端能收到前台子代理的完整事件流（subagent\_start -> agent\_message -> subagent\_stop）。
...
## Task 4: 优化 task\_spawn\_agent 工具定义描述
...
* [x] SubTask 4.1: 更新 `agent.py` 中 `task_spawn_agent` 工具定义的 description
  ...
  * 在 model\_hint 中包含当前正在使用的模型信息，引导 LLM 正确传递
