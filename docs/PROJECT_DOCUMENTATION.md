...
Open-AwA 是一个以 FastAPI 后端和 React 前端构建的 AI Agent 平台。该项目定位为 **OpenClaw 级别的 AI Agent 能力 + 二次元 AI 陪伴的情感体验**，既是一个连接大模型与实际系统操作的执行层框架，也是一个具备角色系统、桌面宠物、TTS 语音、日记、Soul 引擎等陪伴基础设施的二次元 AI 陪伴产品。项目当前已实现聊天调用、技能管理、插件管理、记忆管理、经验提取、提示词配置、行为统计、会话记录采集与计费模块等功能，并提供了一套可独立演进的插件生命周期与调试能力。v1.6 进一步扩展了子代理编排、IM 渠道适配、任务运行时、定时任务、角色系统、用户画像、工作区、编码助手、TTS、终端、自主运行模式、数据仪表盘、收件箱、插件市场等模块，前端页面路由扩展至 30+，后端路由模块扩展至 39 个。

本项目采用**微内核+插件**的分层架构设计，遵循"从对话到执行"的范式转移趋势。系统的核心价值体现在五个维度：本地运行确保数据完全私有、安全可靠；自主执行能力使系统不仅能回答问题，还能执行实际操作；可扩展性通过 Skill、MCP、插件、子代理的灵活扩展实现；多层次安全防护机制保障系统安全可控；情感陪伴能力通过角色人格、桌面宠物、语音合成、日记生成等模块提供有温度的 AI 互动体验。
...
- **OPENAWA_API_KEY 强制校验**：v1.5 引入，未配置时拒绝启动，CLI 工具 bin/generate_api_key.py 写入 .env.local 后立即 chmod 600 消除 TOCTOU 窗口。
