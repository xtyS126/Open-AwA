...
| 命令 | 说明 |
|------|------|
| `python bin/bin/generate_api_key.py` | 生成新密钥并写入 .env.local，已有密钥时拒绝覆盖 |
| `python bin/bin/generate_api_key.py --show` | 仅生成并打印，不写入文件 |
| `python bin/bin/generate_api_key.py --force` | 强制替换已有密钥 |
...
| `DATABASE_URL` | sqlite:///./var/data/openawa.db | 数据库连接 |
...
| `VECTOR_DB_PATH` | var/data/qdrant | ChromaDB持久化路径 |
...
项目根目录提供 `deploy/docker-compose.yml` 和 `Dockerfile`：
...
| 详细技术文档 | [PROJECT_DOCUMENTATION.md](PROJECT_DOCUMENTATION.md) | 完整技术文档 |
