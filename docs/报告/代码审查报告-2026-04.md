...
| `docs/指南/部署与运行说明.md` | ✅ 一致 | 端口、环境变量与 `settings.py` 匹配 |
| `docs/架构/后端架构说明.md` | ✅ 一致 | 模块描述与实际目录对应 |
| `docs/架构/前端架构说明.md` | ✅ 一致 | 页面路由与 `App.tsx` 匹配 |
| `docs/指南/测试说明.md` | ⚠️ 部分不一致 | 测试文件数量描述与实际略有偏差 |
