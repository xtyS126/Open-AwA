...
- **Symptom**:
  - `frontend/src/App.tsx:75` 仅在 RouterProvider 外包一层 `<ErrorBoundary name="Root">`，任何路由内部抛错顶替整个应用壳层
...
- **Symptom**:
  - `frontend/src/shared/api/client.ts:237` `axios.create` 未配置 `timeout`，所有请求默认无超时
...
- **Symptom**:
  - `frontend/src/shared/store/authStore.ts:43` `logout` 仅 `set({ user: null, apiKey: null, isAuthenticated: false })`，未清理 sessionStore / toolCallStore / inboxStore / modelStore
...
- **Symptom**:
  - `frontend/src/shared/hooks/useAppInitialization.ts:244` `init-status` 接口失败时"假定已初始化"`setSystemInitialized(true)`，绕过 setup 引导
...
- **Symptom**:
  - `frontend/src/main.tsx:34` 全局 `window.addEventListener('error', ...)` 与 `window.addEventListener('unhandledrejection', ...)` 未保存 handler 引用
...
|------|----------|---------|------------|------|
| `backend/main.py` | 2 | 1 | 1 | 4 |
| `backend/api/services/` | 1 | 2 | 0 | 3 |
| `backend/api/routes/` | 1 | 3 | 1 | 5 |
| `backend/core/agent.py` | 1 | 1 | 0 | 2 |
| `backend/core/tool_dispatcher.py` | 1 | 0 | 0 | 1 |
| `backend/core/executor.py` | 0 | 1 | 0 | 1 |
| `backend/core/task_runtime/` | 0 | 1 | 0 | 1 |
| `backend/billing/` | 0 | 1 | 0 | 1 |
| `backend/acp_host/` | 0 | 1 | 0 | 1 |
| `frontend/src/shared/api/` | 2 | 1 | 0 | 3 |
| `frontend/src/shared/store/` | 0 | 1 | 0 | 1 |
| `frontend/src/shared/hooks/` | 0 | 1 | 0 | 1 |
| `frontend/src/shared/components/ErrorBoundary/` | 1 | 0 | 0 | 1 |
| `frontend/src/features/chat/` | 1 | 2 | 1 | 4 |
| `frontend/src/features/vibe-coding/` | 1 | 1 | 0 | 2 |
| `frontend/src/features/inbox/` | 0 | 1 | 1 | 2 |
| `frontend/src/features/discussions/` | 0 | 1 | 0 | 1 |
| `frontend/src/App.tsx` + `main.tsx` + `router/` | 1 | 0 | 1 | 2 |
