...
import {
  ...
  fileExperiencesApi,
} from '../services/fileExperiencesApi'
...
function ExperiencePage({ hideHeader = false }: ExperiencePageProps) {
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      const currentExists = fileList.some((item) => item.file_name === selectedFileName)
    ...
    <div className="experience-page">
      {!hideHeader && (
        <div className="page-header">
          <h1>经验文件</h1>
          <button className="btn-secondary" onClick={loadFiles} disabled={loadingList || loadingDetail || saving}>
            刷新列表
          </button>
        </div>
      )}
      ...
      {loadingList ? (
        <div className="loading">正在加载经验文件列表...</div>
      ) : files.length === 0 ? (
        <div className="empty-state">当前没有可用经验文件，请先通过提取流程生成 Markdown 文件。</div>
      ) : (
        <div className="file-experience-layout">
          <aside className="file-list-panel">
            {files.map((file) => (
              <button
                key={file.file_name}
                className={`file-item ${selectedFileName === file.file_name ? 'active' : ''}`}
                onClick={() => handleSelectFile(file.file_name)}
                disabled={loadingDetail || saving}
              >
                <div className="file-item-title">{file.title || file.file_name}</div>
                <div className="file-item-meta">
                  <span>{formatDate(file.updated_at)}</span>
                  <span>{formatFileSize(file.size)}</span>
                </div>
                {file.summary && <div className="file-item-summary">{file.summary}</div>}
              </button>
            ))}
          </aside>
          ...
          <section className="file-editor-panel">
            {loadingDetail ? (
              <div className="loading">正在加载文件内容...</div>
            ) : !selectedFile ? (
              <div className="empty-state">请选择左侧文件查看内容。</div>
            ) : (
              <>
                <div className="editor-header">
                  ...
                  <button className="btn-primary" onClick={handleSave} disabled={!hasUnsavedChanges || saving}>
                    {saving ? '保存中...' : '保存'}
                  </button>
                </div>
