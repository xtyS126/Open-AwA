...
function BillingPage() {
  ...
  if (loading) {
    return <div className="loading">加载计费数据...</div>
  }
  ...
  return (
    <div className="billing-page">
      ...
      <div className="billing-stats-grid">
        ...
        <div className="billing-stat-card">
          <h3>输入Tokens</h3>
          <p className="stat-value">{formatTokens(statistics?.total_input_tokens || 0)}</p>
          <p className="stat-subtitle">输入tokens总量</p>
        </div>
        <div className="billing-stat-card">
          <h3>输出Tokens</h3>
          <p className="stat-value">{formatTokens(statistics?.total_output_tokens || 0)}</p>
          <p className="stat-subtitle">输出tokens总量</p>
        </div>
        <div className="billing-stat-card">
          <h3>API调用次数</h3>
          <p className="stat-value">{statistics?.total_calls || 0}</p>
          <p className="stat-subtitle">总调用次数</p>
        </div>
      </div>
      ...
      <div className="billing-usage-table">
        <h3>用量明细</h3>
            ...
            {usageRecords.length === 0 ? (
              ...
              usageRecords.map((record) => (
                <tr key={record.call_id}>
                  ...
                  <td>
                    <span className={`provider-badge ${record.provider}`}>
                      {record.provider}
                    </span>
                  </td>
                  <td>{record.model}</td>
                  <td>
                    <span className="content-type-badge">{record.content_type}</span>
                  </td>
                  ...
                  <td>
                    ...
                    {record.cache_hit && <span className="cache-badge">缓存</span>}
                  </td>
