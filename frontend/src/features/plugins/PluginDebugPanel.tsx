  ...
  return (
    <div className={styles['debug-panel']}>
      <div className={styles['debug-panel-header']}>
        <span className={styles['debug-panel-title']}>{pluginName} 调试面板</span>
        <div className={styles['debug-panel-controls']}>
          <select
            className={styles['debug-level-select']}
            value={filterLevel}
      ...
      <div className={styles['debug-panel-body']} ref={bodyRef}>
        {loading && entries.length === 0 ? (
          <div className={styles['debug-loading']}>加载中...</div>
        ) : entries.length === 0 ? (
          <div className={styles['debug-empty']}>暂无日志条目</div>
        ) : (
          entries.map((entry, idx) => (
            <div key={idx} className={styles['debug-log-row']}>
