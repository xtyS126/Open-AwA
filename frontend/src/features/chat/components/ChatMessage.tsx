      ...
      <div className={styles['message-content']}>
        {message.role === 'user' && (
          <>
            ...
            {!isCurrentlyStreaming && onEditMessage && (
              <button className={styles['editBtn']} onClick={() => onEditMessage(message.content)} title="编辑消息" aria-label="编辑消息">
                <Pencil size={14} />
              </button>
            )}
          </>
        ...
        {message.role === 'assistant' && !isCurrentlyStreaming && !message.isError && (
          <div className={styles['actionsBar']}>
            <button className={styles['actionBtn']} onClick={handleCopy} title="复制" aria-label="复制">
              {copied ? <Check size={14} /> : <Copy size={14} />}
            </button>
            {onRegenerate && (
              <button className={styles['actionBtn']} onClick={() => onRegenerate(message.id)} title="重新生成" aria-label="重新生成">
                <RefreshCw size={14} />
              </button>
            )}
            {onFeedback && (
