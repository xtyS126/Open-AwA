...
import { __resetConversationListCacheForTests } from '@/features/chat/hooks/useConversationHistory'
...
describe('ChatPage', () => {
  beforeEach(() => {
    ...
    // 重置 useConversationHistory 模块级 stale-while-revalidate 缓存，
    // 避免前一个用例填充的缓存导致本用例跳过 listSessions 调用
  ...
  it('在流式结构化事件中展示悬浮任务面板并支持展开工具详情', async () => {
    ...
    await renderChatPage()
    fireEvent.change(screen.getByPlaceholderText(/输入你的问题|type your question/i), {
