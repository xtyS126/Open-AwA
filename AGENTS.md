...
bin/                # 可执行脚本（dev.bat / deploy.ps1 / install.ps1 / generate_api_key.py / migrate_layout.py / migrate_runtime_data.py）
...
- **Vector DB path is absolute**: 默认值由 `config/runtime_paths.py` 锚定到 `<项目根>/var/data/qdrant`，禁止重新引入依赖工作目录的相对路径
...
## 11. Parallel Subagent Coordination（并行子智能体协作）

AI Agent 在本项目中**可以**派出多个 subagent 同步干活，以加速任务推进。该能力是长期自主迭代的补充手段，而非替代主线闭环。使用前仍需遵守本文件的全部规则约束。

### 11.1 适合派出 subagent 的场景

- 可拆解为**互不重叠写区**的并行任务（如后端模块 A + 前端模块 B 各自独立）
- 多个**独立、自包含**的信息检索或代码探索任务（如并行调研多个技术选型）
- 纯只读、不对主写区产生冲突的辅助分析

### 11.2 派出时必须遵守的边界

- **写区隔离**：每个 subagent 的写文件集合必须不相交，避免合并冲突与互相覆盖
- **不承担关键路径阻塞任务**：若下一步主流程依赖该结果，应自己做而非委托给 submodel
- **遵守本文件全部规则**：subagent 同样受 §1 自主权边界、§2 记忆协议、§6 代码风格、§9 反模式约束（中文注释、禁 emoji 等）
- **不自主 git push**：subagent 的 commit 需遵循 §8，push 仍须由用户确认
- **不共享主上下文**：subagent 独立执行，主 roll-out 必须在委托说明里交代足够的背景（目标、写区、约束、交付物）

### 11.3 集成与验证

- subagent 完成后，主 roll-out 负责**审查并集成**其改动，不可盲目信任
- 集成后仍须走 §3 的 6 步验证闭环（测试 + E2E + 记忆沉淀）
- 若 subagent 改动违反本契约，主 roll-out 须修正或丢弃，不可直接通过验证
