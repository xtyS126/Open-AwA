...
from config.runtime_paths import PLUGINS_DATA_DIR
...
_RUNTIME_PROJECT_ROOT = PLUGINS_DATA_DIR / "bilibili_toolkit_builtin"
...
class BilibiliToolkitAdapter:
    ...
    async def initialize(self) -> None:
        ...
        self._configure_runtime_project_root()
    ...
    def _configure_runtime_project_root(self) -> None:
        """为 vendored 上游组件指定项目外的运行时目录。"""
        configured_root = os.environ.get(_PROJECT_ROOT_ENV, "").strip()
        if configured_root:
            logger.info(
                f"Bilibili Toolkit 使用显式运行时目录: {configured_root}"
            )
            return

        _RUNTIME_PROJECT_ROOT.mkdir(parents=True, exist_ok=True)
        os.environ[_PROJECT_ROOT_ENV] = str(_RUNTIME_PROJECT_ROOT)
        logger.info(
            f"Bilibili Toolkit 运行时目录已设置为: {_RUNTIME_PROJECT_ROOT}"
        )
