...
class BilibiliToolkitBuiltinPlugin(BasePlugin):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> Any:
        ...
        bilibili-toolkit-builtin 内置插件优先通过 ``get_tools()`` 返回的 handler 调度
        （PluginManager 的 ``execute_registered_tool_async`` / ``execute_plugin_async``
        已识别 ``__handler__`` 标记并直接调用 handler）。

        当调用方仍走旧版 ``execute(action=<tool_name>)`` 入口时，本方法提供降级实现：
        从 ``self._tools`` 中查找 name 匹配的工具并调用其 handler，避免直接抛
        ``NotImplementedError`` 导致调用失败。
        """
        action = kwargs.get("action") or (args[0] if args else "")
        if not action:
            raise ValueError(
                "BilibiliToolkitBuiltinPlugin.execute 需要 action 参数指定要调用的工具"
            )
        ...
        for tool_def in self._tools:
            ...
            if not callable(handler):
                raise RuntimeError(
                    f"BilibiliToolkitBuiltinPlugin tool '{action_name}' handler not callable"
                )
            # 移除 action 参数后传递其余 kwargs 给 handler
            handler_kwargs = {k: v for k, v in kwargs.items() if k != "action"}
            return handler(**handler_kwargs)
        raise NotImplementedError(
            f"BilibiliToolkitBuiltinPlugin 不支持未知 action '{action_name}'，"
