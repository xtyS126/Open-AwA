...
class PluginManager:
    ...
    def _get_default_plugins_dir(self) -> str:
        ...
        Returns:
            插件目录的绝对路径。
        """
        # __file__ = backend/plugins/plugin_manager.py；上溯 3 级即为项目根
        default_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "plugins"))
    ...
    def _run_static_security_scan(self, plugin_path: str) -> Dict[str, Any]:
        ...
        """
        return self.security_scanner.scan(plugin_path)
    ...
    def _resolve_plugin_method_and_kwargs(
        ...
        解析顺序：
        1. ``help`` 工具优先路由到 ``BasePlugin.get_help``
        2. tools_registry 中已注册为 ``__handler__`` 模式的工具：直接返回 ``__handler__`` 标记
        3. plugin_instance 上的直接方法
        4. 仅通过 get_tools 暴露的历史工具：自动回落到 ``execute(action=<tool_name>)``
        """
        ...
        # handler 模式优先：tools_registry 中已标记为 __handler__ 的工具
        # 直接返回标记，由 execute_plugin_async 调用 _invoke_plugin_tool_handler
        # 避免落到 plugin.execute(action=<tool_name>) 导致 NotImplementedError
        for tool_def in self.get_plugin_tools(plugin_name):
            if str(tool_def.get("name") or "") != method:
                continue
            if tool_def.get("method") == "__handler__" and callable(tool_def.get("handler")):
                default_params = tool_def.get("default_params")
                merged_kwargs = dict(default_params) if isinstance(default_params, dict) else {}
                merged_kwargs.update(kwargs)
                return "__handler__", merged_kwargs
            # 找到同名工具但不是 handler 模式，退出循环走原有逻辑
            break
    ...
    async def _invoke_plugin_tool_handler(
        self,
        plugin_name: str,
        tool_name: str,
        kwargs: Dict[str, Any],
    ) -> Optional[Dict[str, Any]]:
        """查找并调用插件工具的 handler，返回标准化结果。

        用于 ``execute_plugin_async`` 路径 B 识别 ``__handler__`` 标记后调用 handler，
        避免回落到 plugin.execute(action=<tool_name>) 触发 NotImplementedError。
        ...
        Returns:
            标准化结果字典；工具未找到时返回 None，由调用方决定后续处理
        """
        for tool_def in self.get_plugin_tools(plugin_name):
            if str(tool_def.get("name") or "") != tool_name:
                continue
            ...
            try:
                ...
                if inspect.isawaitable(result):
                    result = await result
                return self._normalize_plugin_execution_result({
                    "status": "success",
                    "result": result,
                })
            except Exception as exc:  # noqa: BLE001 - handler 边界统一捕获
                logger.error(
                    f"Plugin '{plugin_name}' tool '{tool_name}' handler execution failed: {exc}"
                )
                return {
                    "status": "error",
                    "message": f"Tool handler execution failed: {exc}",
                }
        return None
    ...
    async def execute_plugin_async(self, plugin_name: str, method: str, **kwargs) -> Dict[str, Any]:
        ...
        # handler 模式：直接调用工具定义中的 handler，绕过 sandbox 与 plugin_instance 方法检查
        # 适用于通过 get_tools() 暴露 handler 的插件（如 bilibili-toolkit-builtin）
        if resolved_method == "__handler__":
            handler_result = await self._invoke_plugin_tool_handler(
                plugin_name, method, resolved_kwargs
            )
            if handler_result is not None:
                return handler_result
            # handler 未找到时继续走 sandbox 路径返回错误
    ...
    def _normalize_tool_definition(
        ...
        支持三种工具暴露方式：
        1. ``handler`` 字段（推荐）：插件通过 ``get_tools()`` 返回包含 callable handler 的字典，
           handler 不依赖 plugin_instance 上的同名方法，避免工具被丢弃。
           method 字段标记为 ``__handler__``，由 ``execute_registered_tool_async``
           与 ``execute_plugin_async`` 统一识别并直接调用 handler。
        2. 直接方法：plugin_instance 上存在与 tool_name 同名的 callable 属性。
        3. execute 入口（旧插件）：回落到 ``execute(action=<tool_name>)``。
        """
        ...
        # 优先识别 handler 字段：插件通过 get_tools() 暴露的可调用 handler
        # 不依赖 plugin_instance 上的同名方法，避免工具被 _normalize_tool_definition 丢弃
        ...
        if not method_name:
            ...
            elif handler_callable:
                # 工具通过 handler 暴露，不依赖实例方法
                # 设置 __handler__ 标记，execute_registered_tool_async / execute_plugin_async
                # 会识别该标记并直接调用 handler
                method_name = "__handler__"
            else:
                method_name = "execute"
                default_params.setdefault("action", tool_name)

        # handler 模式跳过 hasattr 检查（handler 不在 plugin_instance 上）
        if method_name == "__handler__":
            if not handler_callable:
                logger.warning(
                    f"Plugin '{plugin_name}' tool '{tool_name}' marked as __handler__ but handler not callable"
                )
                return None
        elif not hasattr(plugin_instance, method_name) or not callable(getattr(plugin_instance, method_name)):
            logger.warning(
