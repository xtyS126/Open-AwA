...
from config.runtime_paths import PROJECT_ROOT, STATE_DIR, get_workspace_dir
...
def _load_soul_content() -> str:
    ...
    文件不存在时返回默认人格提示。
    """
    # 候选路径优先级：var/state/SOUL.md（新规范位置）> 旧位置兜底（向后兼容）
    # backend/api/routes/diary.py 的 parents[4] 才是项目根目录
    possible_paths = [
        str(STATE_DIR / "SOUL.md"),
        str(PROJECT_ROOT / "SOUL.md"),
        os.path.join(os.getcwd(), "SOUL.md"),
        str(get_workspace_dir() / "SOUL.md"),
    ]
