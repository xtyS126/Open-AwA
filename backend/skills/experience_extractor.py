...
class ExperienceExtractor:
    """
    经验提取器，从对话中提取结构化经验，支持质量过滤和格式化输出。
    """
    def __init__(self, llm_client=None):
        """
        初始化经验提取器，绑定LLM客户端并设置输出路径。
        """
        self.llm_client = llm_client
        # 路径：项目根 / var / memory_skill
        # 说明：backend/skills/experience_extractor.py 的 parents[2] 才是项目根目录
        self.memory_skill_dir = Path(__file__).resolve().parents[2] / "var" / "memory_skill"
