...
校验解码尺寸后原子安装到 backend/data/pets/builtin/<pet_id>/ 缓存中。
...
# 宠物资产在 Open-AwA 数据目录下的根
PETS_DATA_DIR: Path = Path(__file__).resolve().parents[1] / "data" / "pets"
...
def custom_pet_dir(user_id: str, pet_id: str) -> Path:
    """返回自定义宠物目录：data/pets/<user_id>/<pet_id>/。"""
