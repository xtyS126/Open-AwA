...
def migrate_memory_layers():
    ...
    默认值：semantic（语义记忆层）
    """
    print("[迁移] 开始为现有记忆添加 memory_layer 字段...")
    ...
    with SessionLocal() as session:
        ...
        if migrated >= total:
            print(f"[迁移] 无需迁移，所有 {total} 条记忆已有 memory_layer 字段")
            return
        ...
        session.commit()
        print(f"[迁移] 完成，已更新 {updated} 条记忆的 memory_layer 字段为 'semantic'")
...
def init_decay_config():
    ...
    with SessionLocal() as session:
        for config in default_configs:
            ...
            if existing:
                print(f"[迁移] 跳过已存在的配置: {config['layer']}")
                continue

            new_config = MemoryDecayConfig(**config)
            session.add(new_config)
            print(f"[迁移] 已添加配置: {config['layer']}")
...
def verify_migration():
    ...
    with SessionLocal() as session:
        ...
        print(f"[验证] 总记忆数: {total}")
        print(f"[验证] 已有 memory_layer 字段: {with_layer}")
        ...
        for layer, count in layer_stats:
            print(f"[验证]   {layer}: {count} 条")

        # 检查衰减配置
        config_count = session.query(MemoryDecayConfig).count()
        print(f"[验证] 衰减配置数: {config_count}")


if __name__ == "__main__":
    print("=" * 50)
    print("Open-AwA 数据迁移: 多层记忆架构")
    print("=" * 50)
