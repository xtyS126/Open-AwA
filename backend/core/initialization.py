...
公共函数：
- `get_initialized_marker_path()`：解析标记文件路径（按 INITIALIZED_MARKER_PATH → DATA_DIR → backend/data 优先级）
...
_DEFAULT_MARKER_DIR: Path = Path("backend/data")
...
def get_initialized_marker_path() -> Path:
    ...
    3. `backend/data/.initialized`（相对路径，相对于进程 CWD）
