...
def test_default_data_directory_uses_runtime_plugin_directory():
    """插件默认运行数据不得写入源码插件目录。"""
    plugin_instance = TwitterMonitorPlugin(config={})
    project_root = Path(__file__).resolve().parents[2]

    assert plugin_instance.data_dir == project_root / "var" / "plugins" / "twitter-monitor" / "data"
