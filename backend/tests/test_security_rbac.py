...
from security.rbac import PermissionCheckError, RBACManager
...
class TestEnsureBuiltInRoles:
    ...
    def test_existing_roles_do_not_commit_again(self, db_session):
        """内置角色已齐全时不应产生额外写事务。"""
        rbac = RBACManager(db_session)
        rbac.ensure_built_in_roles()
...
class TestGetRolePermissions:
    ...
    @pytest.mark.asyncio
    async def test_corrupt_permissions_raise_security_error(self, db_session):
        """权限 JSON 损坏时必须中止校验，不能静默降级为允许访问。"""
        rbac = RBACManager(db_session)
        rbac.ensure_built_in_roles()
        role = db_session.query(Role).filter(Role.name == "developer").first()
        role.permissions = "not-json"
        db_session.commit()

        with pytest.raises(PermissionCheckError):
            await rbac.get_role_permissions("developer")

        await rbac.set_user_role("corrupt-role-user", "developer")
        with pytest.raises(PermissionCheckError):
            await rbac.check_permission("corrupt-role-user", "chat:send")
