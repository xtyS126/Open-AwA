...
from plugins.bilibili_toolkit_builtin import adapter as bilibili_adapter  # noqa: E402
...
def test_adapter_sets_canonical_runtime_root(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """未显式覆盖时，vendored 数据必须落在项目 var/plugins 下。"""
    runtime_root = tmp_path / "bilibili_toolkit_builtin"
    monkeypatch.delenv("OPENBILICLAW_PROJECT_ROOT", raising=False)
    monkeypatch.setattr(bilibili_adapter, "_RUNTIME_PROJECT_ROOT", runtime_root)

    adapter = BilibiliToolkitAdapter()
    adapter._configure_runtime_project_root()

    assert runtime_root.is_dir()
    assert os.environ["OPENBILICLAW_PROJECT_ROOT"] == str(runtime_root)
    from openbiliclaw.config import load_config

    assert load_config().data_path == runtime_root / "data"
