...
def test_transcript_defaults_use_canonical_runtime_directory(tmp_path):
    """JSONL 默认路径不得随进程工作目录落入源码树。"""
    writer_default = inspect.signature(JsonlTranscriptWriter.__init__).parameters[
        "base_dir"
    ].default
    replay_default = inspect.signature(replay_transcript).parameters["base_dir"].default

    assert writer_default == TRANSCRIPTS_DIR
    assert replay_default == TRANSCRIPTS_DIR

    writer = JsonlTranscriptWriter("session-1", base_dir=tmp_path)
    writer.append(uuid="message-1", parent_uuid=None, type="user", content="hello")

    assert replay_transcript("session-1", base_dir=tmp_path)[0]["content"] == "hello"
