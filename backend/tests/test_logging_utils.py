...
from config.logging import get_log_file_list, init_logging, query_log_buffer, sanitize_for_logging, set_request_id
from config.runtime_paths import LOG_DIR
...
def test_log_helpers_default_to_runtime_log_directory():
    """日志辅助函数默认读取统一运行时日志目录。"""
    assert get_log_file_list.__defaults__ == (str(LOG_DIR),)
    assert init_logging.__defaults__[3] == str(LOG_DIR)
